...
def clean_html_to_text(html_content):
    """Supprime les balises HTML et garde seulement le texte brut"""
    soup = BeautifulSoup(html_content, "html.parser")
    return soup.get_text(separator="\n")
    
def export_docx(note):
    buffer = io.BytesIO()
    doc = Document()
    doc.add_heading(note.titre, 0)
    doc.add_paragraph(note.contenu)  # HTML affiché comme texte brut
    doc.save(buffer)
    buffer.seek(0)
    return buffer
...
def export_pdf(note):
    ...
    # Titre de la note
    story.append(Paragraph(f"<b>{note.titre}</b>", styles["Title"]))
    
    # Nettoyage du contenu HTML -> texte brut
    plain_text = clean_html_to_text(note.contenu)
    ...
    def creer_note(col, note, bg_color):
        with col:
            with st.container(border=True):
                ...
                # Affichage/édition avec streamlit-quill
                nouveau_contenu = st_quill(
                    value=note.contenu,
                    html=True,
                ...
                with col3:
                    if st.button("🗑️", key=f"del_{note.id}", help="Supprimer", type="secondary"):
                        ...
                        st.rerun()
                                    # Export DOCX
                with col4:
                    if st.download_button("📄 DOCX", key=f"docx_{note.id}", help="Exporter en Word",
                        data=export_docx(note), file_name=f"note_{note.id}.docx"):
                        st.toast("Note exportée en DOCX", icon="📄")

                # Export PDF
                with col5:
                    if st.download_button("📕 PDF", key=f"pdf_{note.id}", help="Exporter en PDF",
                        data=export_pdf(note), file_name=f"note_{note.id}.pdf"):
                        st.toast("Note exportée en PDF", icon="📕")
...
tab1, tab2, tab3, tab4, tab5, tab6 , tab7,tab8= st.tabs([
    ...
    "📈 Dashboard & Calendrier", "⚙️ Développement", "📌 Notes", "📌 Notes new","🗓️ Calendrier"
])
...
with tab7:
    st.header("📌 new Notes visuelles (Post-it)")

    # Ajouter une nouvelle note
    with st.expander("➕ Ajouter une note"):
        with st.form("form_notenew"):
            titre = st.text_input("Titre", "Nouvelle note")

            # Remplace st.text_area par st_quill
            contenu = st_quill(
    ...
    st.subheader("📝 new Notes actives")
